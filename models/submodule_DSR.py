...
class DSR_feature_extraction(nn.Module):
...
class DSR_loss():
    def __init__(self, res_output, output, lr_pyramid, disp_gt, mask,mask_d):
        self.res_output = res_output
        self.weight = [1,0.7,0.5,0.3]
        self.lr_pyramid = [lp.detach() for lp in lr_pyramid]
        self.disp_gt = disp_gt
        self.mask = mask
        self.mask_d = mask_d
        self.output = output
        self.res_gt = [self.disp_gt[i] * mask_d[i] - self.lr_pyramid[i] * mask_d[i] for i in range(4)]
        self.res_gt = [g.detach() for g in self.res_gt]
        o_loss_list = [F.smooth_l1_loss(self.lr_pyramid[i]*self.mask[i], self.disp_gt[i]*self.mask[i], size_average=True)*self.weight[i] for i in range(4)]
    ...
    def res_loss(self):
        l1_res_loss_list = [F.smooth_l1_loss(self.res_output[i]*self.mask[i], self.res_gt[i]*self.mask[i], size_average=True) * self.weight[i] for i in range(4)]
        self.l1_res_loss = sum(l1_res_loss_list)
        
        self.diff_loss = torch.exp(self.l1_res_loss - self.o_loss)

        SSIM_loss_list = [torch.mean(self.SSIM(self.output[i] * self.mask[i], self.disp_gt[i] * self.mask[i]))*self.weight[i] for i in range(4)]
        self.SSIM_loss = sum(SSIM_loss_list)
        
        lr_l1_loss_list = [F.smooth_l1_loss(self.output[i]*self.mask[i], self.lr_pyramid[i]*self.mask[i],size_average=True)*self.weight[i] for i in range(4)]
    ...
    def global_loss(self):
        l1_full_loss_list = [F.smooth_l1_loss(self.output[i]*self.mask_d[i], self.disp_gt[i]*self.mask_d[i], size_average=True)*self.weight[i] for i in range(4)]
        self.true_l1 = l1_full_loss_list[0]
        self.l1_full_loss = sum(l1_full_loss_list)
        SSIM_full_loss_list = [torch.mean(self.SSIM(self.output[i]*self.mask_d[i], self.disp_gt[i]*self.mask_d[i]))*self.weight[i] for i in range(4)]
