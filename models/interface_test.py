...
class HRSCnet(nn.Module):
    ...
    def cost_volume(self, refimg_fea, targetimg_fea, maxdisp, leftview=True):
        ...
        full_mask = Variable(torch.cuda.FloatTensor(refimg_fea.size()[0],refimg_fea.size()[1],refimg_fea.size()[2],refimg_fea.size()[3]).fill_(5.))
    ...
    def forward(self,left,right):
        ...
        cost_aggregation = F.upsample(cost_aggregation, [self.disp_reg.disp.shape[1], left.size()[2],left.size()[3]], mode='trilinear').squeeze(1)
        print(cost_aggregation.size())
